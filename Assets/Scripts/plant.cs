...
public class plant : MonoBehaviour
{
   ...
   public int state;
    ...
    public void plantSeed(int x, int y)
    {
        ...
        g.GetComponent<grow>().x = x;
        g.GetComponent<grow>().y = y;
        g.GetComponent<grow>().Name = name;
        g.GetComponent<lootSystem>().isCrop = true;
        g.GetComponent<lootSystem>().plant = this;

        dataStatic.Instance.plants[x, y] = g.GetComponent<grow>();
    ...
    public void plantSeed(int x, int y, string name)
    {
        if(grid == null)
        {
            grid = GameObject.Find("gridfarm").GetComponent<GridFarming>();
